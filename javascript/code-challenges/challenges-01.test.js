...
const allUpperCase = ( array ) => {
  let UC =[];
  array.forEach( value => {
    let editUcase = value.toUpperCase();
    UC.push( editUcase );
  } 
  );
...
------------------------------------------------------------------------------------------------ */
const greeting = ( UcWithSymbol ) => {
  let edit = UcWithSymbol.toUpperCase() + '!';
  return edit;
};

const speaker = ( UcaseWithSymbol, callback ) => {
  let edit = [];
  UcaseWithSymbol.forEach(editword=> {
    let afterEdit = callback ( editword );
    edit.push( afterEdit );
  }
  );
...
const addNumbers = ( number, arrayNumbers, numberOfTimes, callbackFunction ) => {
  for ( let count = 0; count<numberOfTimes; count++ ){
    callbackFunction( arrayNumbers,number );
  }
  return arrayNumbers;
};
...
------------------------------------------------------------------------------------------------ */
const createList = ( availa ) => {
  let array = [];
  availa.forEach ( names=> {
    if ( names.available === true ) { array.push(names.name); }
  } );
  return array;
};
